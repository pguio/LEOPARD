...
if (ic1 < mpnw2 .or. ic2 < mpnw2) then
  write (6, 2) mpnw2
2 format ('*** BESSELKNR: Pi and Egamma must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (527)
  call mpabrt (527)
endif
...
if (ic1 < mpnw2 .or. ic2 < mpnw2) then
  write (6, 2) mpnw2
2 format ('*** BESSELYNR: Pi and Egamma must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (538)
  call mpabrt (538)
endif
...
if (ic1 < mpnw1) then
  write (6, 2) mpnw1
2 format ('*** BESSELYR: Pi must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (544)
  call mpabrt (544)
endif
...
elseif (i1 < 0 .and. i2 <= 0) then
  write (mpldb, 3)
3 format ('*** BESSELYR: First argument < 0 and second argument <= 0')
!   call mpabrt (545)
  call mpabrt (545)
endif
...
if (ic1 < mpnw1) then
  write (6, 2) mpnw1
2 format ('*** ERFR: Pi must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (551)
  call mpabrt (551)
endif
...
write (mpldb, 3)
3 format ('*** ERFR: End loop error 1')
! call mpabrt (552)
call mpabrt (552)
...
write (mpldb, 4)
4 format ('*** ERFR: End loop error 2')
! call mpabrt (553)
call mpabrt (553)
...
if (ic1 < mpnw1) then
  write (6, 2) mpnw1
2 format ('*** ERFCR: Pi must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (555)
  call mpabrt (555)
endif
...
write (mpldb, 4)
4 format ('*** ERFRC: End loop error 1')
! call mpabrt (557)
call mpabrt (557)
...
!   except that for values within 10^-10 of an integer (on the low side), a
!   separate algorithm, based on DLMF 5.7.3, is used.
...
integer, parameter:: itrmx = 100000
real (mpdknd), parameter:: al2 = 0.69314718055994530941723212145817657e0_mpdk
real (mpdknd), parameter:: dmax = 1e8_mpdk, dsmall = 1e-10_mpdk
integer i, ic1, ic2, isgn, i1, i2, j, k, nt, n1, n2, n3
...
! type (mp_real) term1, term2
integer (mpiknd) term1(0:mpnw+6), term2(0:mpnw+6)
! type (mp_real) t1, t2, t3, t4, t5
integer (mpiknd) t1(0:mpnw+6), t2(0:mpnw+6), t3(0:mpnw+6), t4(0:mpnw+6), t5(0:mpnw+6)
! type (mp_real) t6, t7
integer (mpiknd) t6(0:mpnw+6), t7(0:mpnw+6)
integer mpnw1
...
call mpinitwds (sum2(0:mpnw+6), mpnw+6-5)
call mpinitwds (tn(0:mpnw+6), mpnw+6-5)
call mpinitwds (term1(0:mpnw+6), mpnw+6-5)
call mpinitwds (term2(0:mpnw+6), mpnw+6-5)
call mpinitwds (t1(0:mpnw+6), mpnw+6-5)
call mpinitwds (t2(0:mpnw+6), mpnw+6-5)
call mpinitwds (t3(0:mpnw+6), mpnw+6-5)
call mpinitwds (t4(0:mpnw+6), mpnw+6-5)
...
! ic2 = mpwprecr (mpegammacon)
ic2 = mpwprecr (mpegammacon(0:))
if (ic1 < mpnw1 .or. ic2 < mpnw1) then
  write (6, 3) mpnw1
3 format ('*** BESSELKNR: Pi and Egamma must be initialized to',i6,' words by calling mpinit.')
!   call mpabrt (562)
  call mpabrt (562)
endif
...
!   If tn is very close to 1, then use a separate algorithm; otherwise use Potter.

!mp prec="mpnwm"
! tc1 = tn - f1 + dsmall
call mpsub (tn(0:mpnw+6), f1(0:mpnw+6), mpt1(0:mpnw+6), mpnwm)
call mpdmc (dsmall, 0, mpt3(0:mpnw+6), mpnwm)
call mpadd (mpt1(0:mpnw+6), mpt3(0:mpnw+6), mpt2(0:mpnw+6), mpnwm)
call mpeq (mpt2(0:mpnw+6), tc1(0:mpnwm+5), mpnwm)
! ic1 = mpsgn (tc1)
mpi1 = mpsgn (tc1(0:mpnwm+5))
ic1 = mpi1
...
if (ic1 > 0) then
!   t6 = tn - f1
  call mpsub (tn(0:mpnw+6), f1(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
  call mpeq (mpt1(0:mpnw+6), t6(0:mpnw+6), mpnw1)
!   term1 = - log (tn)
  call mplog (tn(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
  call mpneg (mpt1(0:mpnw+6), mpt2(0:mpnw+6), mpnw1)
  call mpeq (mpt2(0:mpnw+6), term1(0:mpnw+6), mpnw1)
!   t2 = 1.d0 - mpegammacon
  call mpdmc (1.d0, 0, mpt2(0:mpnw+6), mpnw1)
  call mpsub (mpt2(0:mpnw+6), mpegammacon(0:), mpt1(0:mpnw+6), mpnw1)
  call mpeq (mpt1(0:mpnw+6), t2(0:mpnw+6), mpnw1)
!   term2 = t6 * t2
  call mpmul (t6(0:mpnw+6), t2(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
  call mpeq (mpt1(0:mpnw+6), term2(0:mpnw+6), mpnw1)
!   sum1 = mprealdp (0.d0)
  call mprealdp (0.d0, mpt1(0:mpnw+6), mpnw1)
  call mpeq (mpt1(0:mpnw+6), sum1(0:mpnw+6), mpnw1)
!   t2 = t6
  call mpeq (t6(0:mpnw+6), t2(0:mpnw+6), mpnw1)
  isgn = -1
...
!     t2 = t6 * t2
    call mpmul (t6(0:mpnw+6), t2(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
    call mpeq (mpt1(0:mpnw+6), t2(0:mpnw+6), mpnw1)
!     call mpzetaintr (k, t7, mpnw1)
    call mpzetaintr (k, t7, mpnw1)
!     t3 = t7 - f1
    call mpsub (t7(0:mpnw+6), f1(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
    call mpeq (mpt1(0:mpnw+6), t3(0:mpnw+6), mpnw1)
!     t4 = t2 * t3
    call mpmul (t2(0:mpnw+6), t3(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
    call mpeq (mpt1(0:mpnw+6), t4(0:mpnw+6), mpnw1)
!     t5 = d1 * t4 / d2
    call mpmuld (t4(0:mpnw+6), d1, mpt1(0:mpnw+6), mpnw1)
    call mpdivd (mpt1(0:mpnw+6), d2, mpt2(0:mpnw+6), mpnw1)
    call mpeq (mpt2(0:mpnw+6), t5(0:mpnw+6), mpnw1)
!     sum1 = sum1 + t5
    call mpadd (sum1(0:mpnw+6), t5(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
    call mpeq (mpt1(0:mpnw+6), sum1(0:mpnw+6), mpnw1)
!mp prec="mpnwm"
!     tc1 = abs (t5) - eps * abs (sum1)
    call mpabs (t5(0:mpnw+6), mpt1(0:mpnw+6), mpnwm)
...
!     ic1 = mpsgn (tc1)
  ...
  write (6, 4)
4 format ('*** GAMMAR: Loop end error 1')
!   call mpabrt (563)
  call mpabrt (563)
...
!mp prec="mpnw1"
!   t2 = term1 + term2 + sum1
  call mpadd (term1(0:mpnw+6), term2(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
  call mpadd (mpt1(0:mpnw+6), sum1(0:mpnw+6), mpt2(0:mpnw+6), mpnw1)
  call mpeq (mpt2(0:mpnw+6), t2(0:mpnw+6), mpnw1)
!   t3 = exp (t2)
  call mpexp (t2(0:mpnw+6), mpt1(0:mpnw+6), mpnw1)
  call mpeq (mpt1(0:mpnw+6), t3(0:mpnw+6), mpnw1)
!mp prec="mpnw"
!   z = t1 * t3
  call mpmul (t1(0:mpnw+6), t3(0:mpnw+6), mpt1(0:mpnw+6), mpnw)
  call mpeq (mpt1(0:mpnw+6), z(0:), mpnw)
else
...
!     ic1 = mpsgn (tc1)
  ...
  write (mpldb, 5)
  5 format ('*** GAMMAR: End loop error 2')
!   call mpabrt (564)
  call mpabrt (564)
...
!     ic1 = mpsgn (tc1)
  ...
  write (mpldb, 6)
6 format ('*** GAMMAR: End loop error 3')
!   call mpabrt (565)
  call mpabrt (565)
