...
def dist_bimax(vpar, vper, n, m, beta_par, beta_per, drift):
	bimax = np.exp(-n*(vpar-drift)**2/beta_par/m -n*vper**2/beta_per/m)*\
            n**1.5 /(m**1.5 *np.pi**1.5 *beta_per*np.sqrt(beta_par))
	return bimax.ravel()
...
for ispecies in range(0, Nspecies):
	...
	data = dist_bimax(vpara2,vperp2,dens[ispecies],mu[ispecies],
           beta_para[ispecies],beta_perp[ispecies],vdrift[ispecies])
